...
if (!empty($variablecontrol)) {
    if (($variablecontrol != "list")) {
        if (empty($provincia_id)) {
            $evento_id = $_POST['id'];
            $localidad_id = DevuelveValor($evento_id, 'localidad_id', 'alojamientos', 'id');
...
new Scaffold(
        "editable", "alojamientos", 30, array('descripcion'), array('empresa_id'), // Campos a ocultar en el formulario
