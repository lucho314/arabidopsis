...
CREATE   TRIGGER  IMPUTAR_PAGOS2 
 ...
 FOR EACH ROW 
   ...
   IF(NEW.numero_cuota = 1) THEN
		...
		SET @FECHA:= (SELECT fecha_vencimiento from tarjeta_de_creditos where id=NEW.tarjeta_id and (date_format(`fecha_vencimiento`, '%Y-%m') = date_format(@FECHA_SALIDA, '%Y-%m')) and date_format(`fecha_vencimiento`, '%d') >= date_format(@FECHA_SALIDA, '%d'));
