 ...
 */    
	...
	*/
	...
	var $singular = array ('estacionamiento','audio','aire_acondicionado','pantalla_y_proyector','habilitacion_municipal','seguro_civil','destino','con_comprobante');	// las fk
	var $plural = array ('si_nos','si_nos','si_nos','si_nos','si_nos','si_nos','localidads','si_nos'); //las tabas correspondientes a las fk anteriores
...
// fin configuracion
	...
	function list_table($msg = NULL, $where = null){
		...
		while($i < mysql_num_fields($select)){
			...
			$i++;
                 ...
                 $col_extra='';
		$page .= "<th></th><th></th><th></th>$col_extra</tr></thead><tbody>";
	...
	*/
	function new_row(){
		...
		//echo '<h2><br>';
		$page .= '<form action="'.$_SERVER['PHP_SELF'].'#'.$this->filter[1].'" method="POST" name="crear_'.$this->filter[1].'" id="crear_'.$this->filter[1].'" enctype="multipart/form-data" onsubmit="return creardescripcion()">'
			...
			if($column->name != 'id'){
				...
				if(($posterior == '_maestro_id') AND ($this->tipo_comprobante == $pre)){
					...
					$page .= '
                                ...
                                /*borrar ->*/   $page .= '
                                        <td class="tabla_'.$column->name.'"><input type="hidden" name="'.$column->name.'" id="'.$column->name.'" value="'.$this->usuario_id.'" /></td>';
					//$page .= '<td align="right"  valign="top" width="200"><strong>'.$this->build_friendly_names($column->name).':</strong></td><td class="tabla_'.$column->name.'"></td>';
					...
					$page .= '
                                            ...
                                            <strong>'.$this->build_friendly_names($column->name).':</strong>
                                                ...
                                                <td class="tabla_'.$column->name.'">
                                                <input required="required" type="text" name="'.$column->name.'" id="'.$column->name.'" />
                                                <img src="images/helpicon.gif" title="'.$comment.'" alt="'.$comment.'">';
					}else{
				...
				*/}else{
					...
					$page .= '
                                            ...
                                            <td class="tabla_'.$column->name.'">
                                                <input '.$clase.' type="text" name="'.$column->name.'" id="'.$column->name.'" value="'.$valor_campo.'" size="35" />
                                                <img src="images/helpicon.gif" title="'.$comment.'" alt="'.$comment.'">
                                            </td>';
				}
			}
...
//				else { $clase = ':required'; }
				...
				if((substr($field->name, -3) == '_id') AND ($field->name != 'usuario_id')){
					...
					if($this->mostrar=='editable'){
					$page .= '<td align="right"  width="150" class="tabla_'.$field->name.'"><strong>'.$this->build_friendly_names($field->name).':</strong></td><td class="tabla_'.$field->name.'"><input class="'.$clase.'" class=":required" type="text" name="'.$field->name.'" id="'.$field->name.'" value="'.$row[$i].'" />';
					}else{
					$page .= '<input type="hidden" name="'.$field->name.'" id="'.$field->name.'" value="'.$row[$i].'" />';
					}
				}else{
					$page .= '<td align="right" class="tabla_'.$field->name.'"><strong>'.$this->build_friendly_names($field->name).':</strong></td><td class="tabla_'.$field->name.'"> <input class="'.$clase.'" class=":required"  type="text" name="'.$field->name.'" id="'.$field->name.'" value="'.$row[$i].'" size="35" /></td>';
				}
